...
/**
 * Resolve nome/username de um usuário (id emitido pelo NimbusCore) para exibição em
 * campos de auditoria (createdBy/updatedBy) das respostas da API do Cardsync.
 *
 * Estratégia: cache local (cs_user_directory) + busca sob demanda no NimbusCore quando
 ...
 * resto do uptime do container - uma troca de nome no NimbusCore só refletia depois do
...
public class UserDirectoryService {
  ...
  // servir de cache de longo prazo - 24h fazia uma troca de nome no NimbusCore demorar até 24h
  // pra aparecer.
  private static final Duration STALE_AFTER = Duration.ofMinutes(1);

  private final UserDirectoryRepository repository;
  private final NimbusCoreInternalClient client;

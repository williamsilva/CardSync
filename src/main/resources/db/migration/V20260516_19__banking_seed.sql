INSERT INTO cs_bank(id, name, code, active, created_at, updated_at, created_by_id) VALUES
    (UUID_TO_BIN(UUID()),'Banco Do Brasil', '001', 0, NOW(), NOW(),(SELECT id FROM cs_users WHERE user_name = 'suporte@cardsync.com.br')),
    (UUID_TO_BIN(UUID()),'Banrisul', '41', 0, NOW(), NOW(),(SELECT id FROM cs_users WHERE user_name = 'suporte@cardsync.com.br')),
    (UUID_TO_BIN(UUID()),'Bradesco', '237', 1, NOW(), NOW(),(SELECT id FROM cs_users WHERE user_name = 'suporte@cardsync.com.br')),
    (UUID_TO_BIN(UUID()),'Caixa Econômica Federal', '104', 0, NOW(), NOW(),(SELECT id FROM cs_users WHERE user_name = 'suporte@cardsync.com.br')),
    (UUID_TO_BIN(UUID()),'HSBC', '399', 0, NOW(), NOW(),(SELECT id FROM cs_users WHERE user_name = 'suporte@cardsync.com.br')),

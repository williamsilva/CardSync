-- =====================================================================
-- Índices para acelerar o dashboard de gerenciamento.
-- Postgres
--
-- O dashboard agrega cs_transaction_acq (vendas/taxas), cs_credit_order
-- (pagamentos) e cs_adjustment (débitos), filtrando por company/acquirer/flag
-- e por faixa de data, agrupando por uma dessas dimensões.
--
-- Sem índices compostos, cada agregação faz full scan + filesort sobre
-- centenas de milhares de linhas — daí a lentidão (e o "carregando eterno"
-- no agrupamento por data, que ainda usa função sobre a data).
--
-- Postgres já tem CREATE INDEX IF NOT EXISTS nativo — sem precisar de
-- variável de sessão/DDL dinâmico.
-- =====================================================================

-- cs_transaction_acq: filtro por data + dimensões de agrupamento
CREATE INDEX IF NOT EXISTS idx_cs_tx_acq_dash_date ON cs_transaction_acq (sale_date, company_id, acquirer_id, flag_id);

CREATE INDEX IF NOT EXISTS idx_cs_tx_acq_dash_company ON cs_transaction_acq (company_id, sale_date);

CREATE INDEX IF NOT EXISTS idx_cs_tx_acq_dash_acquirer ON cs_transaction_acq (acquirer_id, sale_date);

CREATE INDEX IF NOT EXISTS idx_cs_tx_acq_dash_modality ON cs_transaction_acq (modality, sale_date);

-- cs_credit_order: pagamentos por data + dimensões
CREATE INDEX IF NOT EXISTS idx_cs_credit_order_dash_date ON cs_credit_order (release_date, company_id, acquirer_id, flag_id);

-- cs_adjustment: débitos por motivo + data + dimensões
CREATE INDEX IF NOT EXISTS idx_cs_adjustment_dash ON cs_adjustment (adjustment_reason, adjustment_date, company_id, acquirer_id, rv_flag_adjustment_id);

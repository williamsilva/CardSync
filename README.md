...
usuários/grupos/permissões agora vivem no **NimbusCore** — o Authorization Server
central de todas as aplicações Nimbus. O Cardsync ficou só com:

- BFF (`/bff/**`): sessão, cookies, CSRF, proxy — `oauth2Login` contra o NimbusCore externo.
- Resource Server (`/api/**`): valida o JWT emitido pelo NimbusCore (JWKS remoto via
...
## Rodar (DEV)
Requer o NimbusCore rodando (padrão `http://localhost:9090`) para login/JWKS funcionarem.
...
- `CARDSYNC_ISSUER` (URL pública do NimbusCore)
- `CARDSYNC_SPA_BASE_URL`
- `CARDSYNC_ALLOWED_ORIGIN`
- `CARDSYNC_BFF_CLIENT_SECRET` (mesmo valor configurado no client `cardsync-bff` do NimbusCore)
- `CARDSYNC_COOKIE_DOMAIN` (obrigatório, sem default — domínio PAI compartilhado com a SPA,
  não o do próprio CardsyncServer, ver comentário em `.env.railway.example`)
- `NIMBUS_CORE_BASE_URL`, `NIMBUS_INTERNAL_API_SECRET` (mesmo valor configurado no NimbusCore)
...
- Banco é **PostgreSQL** (não MySQL) — se usar o Postgres do próprio Railway, copie as
  ...
  direto com `${{Postgres.PGHOST}}` etc.) — em um banco/schema separado do NimbusCore.
- **Domínio customizado no Railway**: ao configurar o Custom Domain nas Networking Settings,
  confirme que o "Target Port" está em **9091** (a porta que o Dockerfile expõe/a aplicação
  usa de fato, ver `server.port` acima) — deixar no padrão errado (ex.: 80) resulta em 502
  "Application failed to respond" mesmo com o container rodando normalmente.
